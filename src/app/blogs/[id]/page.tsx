...
async function Page({ params }: { params: { id: string } }) {
  ...
  return (
    <div
      className={`flex justify-center mt-2 md:mt-0 overflow-x-hidden max-w-5xl mx-auto text-base ${lato.className}`}
    >
      <div className="flex flex-col w-full gap-1 md:gap-0">
        <div className="mx-2 md:m-4 flex flex-col gap-2 relative items-center justify-center">
          <div className="bg-black w-full rounded-lg">
            <img
              ...
              alt=""
              className="rounded-lg w-full h-[30vh] md:h-[65vh] opacity-40 bg-black"
            />
          </div>
          <h1 className="text-2xl md:text-6xl text-white absolute font-semibold">
            {blog?.title}
          </h1>
        </div>
        ...
        <div className="bg-zinc-100 rounded-md mx-4 my-2 p-4">
          {parse(blog!.content)}
        </div>
      </div>
    </div>
